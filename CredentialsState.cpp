...
CredentialsState::CredentialsState(std::unique_ptr<GameState>& currentState, sf::RenderWindow& window) : currentState(currentState) {
    ...
    std::string menuItemNames[] = { "Designed and created by:", "Pawel Szewczuk", "Return" };
...
void CredentialsState::handleInput(sf::RenderWindow& window) {
            ...
            for (size_t i = 0; i < menuItems.size(); i++) {
                if (menuItems[i].getGlobalBounds().contains(mousePos.x, mousePos.y)) {
                    ...
                    case 1:
                        break;
                    case 2: // "Return"
                        switchState(currentState, std::make_unique<MenuState>(currentState, window));
                        break;
                    default:
                        break;
                    }
...
void CredentialsState::update(sf::RenderWindow& window) {
...
void CredentialsState::handleMusic(sf::Music& music) {
